...
$$\hat{P}_{bo}$$是所有N-gram的打折后的概率。如果某个N-gram在训练数据中出现过，那么就可以使用打折(包括Witten-Bell)方法计算其概率。如果没有出现，那么把history变短，计算
$$
\hat{P}_{bo}(⋅\vert w_2…w_{k−1})
$$，
然后乘以
$$
\alpha(w_{2}\ldots w_{k-1})
$$，
也就是
$$
w_{2}\ldots w_{k-1}
$$这个history打折多出来的那些概率。注意`$\alpha$`并不是自由的参数，所有的N-gram的概率估计出来之后，它就固定了，它只是一个归一化用的常量。
